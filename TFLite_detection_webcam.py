...
df = pd.read_csv('/Users/edgaryepez/Developer/AmericanShip/tflite1/objdetection/CustomersView.csv')
...
def matching(image):
    ...
    for index, row in df.iterrows():
        ...
        if (re.search(pattern, text, re.IGNORECASE) and (re.search(pattern2, text, re.IGNORECASE) or re.search(pattern3, text, re.IGNORECASE))) or (re.search(pattern2, text, re.IGNORECASE) and re.search(pattern3, text, re.IGNORECASE)):
            ...
            # playsound(r'/Users/edgaryepez/Developer/AmericanShip/tflite1/Correct_Answer_sound_effect.mp3')
...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            if matching(image):
                print(True)
                # savepath = os.path.join('/Users/edgaryepez/AmericanShip/FoundMatch', f'{datetime.now()}.jpg')
